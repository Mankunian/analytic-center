    ...
    columnDefs: [
      ...
      },
      {
        ...
        cellTemplate: "<div class=\"ui-grid-cell-contents ng-binding ng-scope\" ng-style=\"{'padding-left': grid.options.treeIndent * row.treeLevel + 'px'}\">" +
          ...
          "src='./img/folder.png' " +
          "alt=''>{{COL_FIELD CUSTOM_FILTERS}} - (TreeLevel) {{row.treeLevel}}</div>"
      },
      {
        name: 'id',
        width: '*',
  ...
  $scope.toggleFirstRow = function (index, treeLevel, row) {
    ...
    if (treeLevel === 0) {
      ...
      $http({
        ...
        var arr = $scope.gridOptions.data;
        value.data.forEach(function (statusData) {
          arr[0]['children'][0]['children'].push(statusData);
        });

        writeoutNode(arr, 0, $scope.gridOptions.data);
